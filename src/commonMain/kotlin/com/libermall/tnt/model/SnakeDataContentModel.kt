...
@Serializable
sealed interface SnakeDataContentModel {
    val value: String

    fun asSnakeData() = value.encodeToByteArray()
        .asIterable()
        .chunked((BitString.MAX_LENGTH / 8) - 1)
        .map { it.toByteArray() }
        .foldRight(null) { bytes, acc: SnakeData? ->
            if (acc == null) {
                SnakeDataTail(BitString(bytes))
            } else {
                SnakeDataCons(BitString(bytes), acc)
            }
        }
}
